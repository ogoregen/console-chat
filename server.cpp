...
void onRequest(librg_event* event){
	...
	if(pass != 42) librg_event_reject(event);
}

void onAcception(librg_event* event){

	std::cout << (unsigned int)event->peer << " connected. ";
	librg_message_send_to(&ctx, LIBRG_EVENT_LAST + 2, event->peer, &id, sizeof(id));
	id++;
	...
	librg_message_send_to(&ctx, LIBRG_EVENT_LAST + 3, event->peer, data->rawptr, librg_data_get_wpos(data));
...
void onDisconnection(librg_event* event){
...
void onMessage2(librg_message* message){ 
	...
	//redirect the message to every peer except the sender -v-

	librg_data* data = librg_data_init_new();

	librg_data_wu32(data, peerNames[id].length() + 1);
	librg_data_wptr(data, &peerNames[id][0], sizeof(char) * (peerNames[id].length() + 1));

	librg_data_wu32(data, length);
	librg_data_wptr(data, incoming, sizeof(char) * length);

	librg_message_send_except(&ctx, LIBRG_EVENT_LAST + 1, message->peer, data->rawptr, librg_data_get_wpos(data));
	librg_data_free(data);

	std::fstream file;
	file.open("history.txt", std::ios_base::app);
	file << peerNames[id] << ": " << incoming << std::endl;
	file.close();
}

void onMessage1(librg_message* message){
...
int main(){
	...
	librg_event_add(&ctx, LIBRG_CONNECTION_DISCONNECT, onDisconnection);
